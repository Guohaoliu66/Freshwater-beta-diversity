##########
Integrating beta diversity (FD) with predictor variables
##########
...
data_dir <- "raw data"
beta_dir <- "D:/Desktop/Beta/R/output_sor_functional_parts/"
...
for (i in seq_along(file_paths)) {
  df <- read_excel(file_paths[i], sheet = "coordinates")
  names(df)[1:3] <- c("site", "X", "Y")
  df <- df %>%
    mutate(site = as.character(site), X = as.numeric(X), Y = as.numeric(Y)) %>%
    drop_na()
...
tp_all <- list()
for (i in seq_along(file_paths)) {
  df <- read_excel(file_paths[i], sheet = "environment")
  names(df)[1] <- "site"
  df <- df %>%
    select(site, TP) %>%
...
beta_paths <- list.files(beta_dir, pattern = "_functional_beta\\.rds$", full.names = TRUE)
beta_names <- tools::file_path_sans_ext(basename(beta_paths)) %>% str_remove("_functional_beta")
names(beta_paths) <- beta_names

beta_matrices <- purrr::map(beta_paths, ~ {
  x <- readRDS(.x)
  list(
    total = as.matrix(x$funct.beta.sor),
    turnover = as.matrix(x$funct.beta.sim),
    nestedness = as.matrix(x$funct.beta.sne)
  )
})
...
dataset_var <- read.csv("input_file/dataset_var.csv") %>%
  rename(dataset = Dataset)

input_info <- dataset_var %>%
  select(dataset, Organism:Ave_Forest)

model_data_FD <- model_data_FD %>%
  left_join(input_info, by = "dataset")

#write_csv(model_data_FD, "output_file/model_data_sorensen_functional_parts.csv")
```

# Compile FD beta and Climate
```{r}
library(raster)
model_data <- read_csv("output_file/model_data_sorensen_functional_parts.csv")
...
Elevation_raster <- raster('input_file/climate/wc2.1_30s_elev.tif')   # no use
...
# write_csv(model_data, file = "output_file/model_data_sorensen_functional_parts.csv")
```

# Compile FD beta and Landuse
```{r}
pacman::p_load(tidyverse, readxl, glue, fossil, sf, raster, exactextractr, pbapply)

model_data <- read_csv("output_file/model_data_sorensen_functional_parts.csv")
colnames(model_data)[colnames(model_data) == "beta_turnover"] <- "turnover"
colnames(model_data)[colnames(model_data) == "beta_nestedness"] <- "nestedness"
...
land_types <- list(
  ...
  Forest   = "input_file/land_use/GDLC/PROBAV_LC100_global_v3.0.1_2019-nrt_Tree-CoverFraction-layer_EPSG-4326.tif"
)
...
for (land in names(land_types)) {
 ...
 write_csv(data_selected, file = "output_file/data_selected_sorensen_functional_parts.csv")
```

# Optimal Land use buffer FD
```{r}
data <- read_csv("output_file/model_data_sorensen_functional_parts.csv")
